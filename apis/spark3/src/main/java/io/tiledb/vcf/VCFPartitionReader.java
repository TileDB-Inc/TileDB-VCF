...
public class VCFPartitionReader implements PartitionReader<ColumnarBatch> {
  ...
  public VCFPartitionReader(
    ...
    log =
        Logger.getLogger(
            VCFPartitionReader.class.getName()
                + " (stage:"
                + task.stageId()
                + "/taskID:"
                + task.partitionId()
  ...
  private void allocateAndSetBuffer(String fieldName, String attrName, long attributeBufferSize) {
    ...
    if (valueVector instanceof ListVector) {
      ...
      data =
          arrowData.nioBuffer(
              0, (int) arrowData.capacity()); // TODO check again here, not safe probably
    ...
    if (info.isList) {
      ...
      ByteBuffer listOffsets = arrowListOffsets.nioBuffer(0, (int) arrowListOffsets.capacity());
    ...
    ByteBuffer bitmap = arrowBitmap.nioBuffer(0, (int) arrowBitmap.capacity());
    if (info.isNullable) {
      vcfReader.setBufferValidityBitmap(attrName, bitmap);
    } else {
      int nbytes = (int) arrowBitmap.capacity();
      for (int i = 0; i < nbytes; i++) {
