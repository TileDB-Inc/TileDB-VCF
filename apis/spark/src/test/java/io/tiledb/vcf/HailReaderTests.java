...
public class HailReaderTests extends SharedJavaSparkSession {
  ...
  public void hailReader() {
    ...
    TileDBHailVCFReader reader =
        TileDBHailVCFReader.build(testSampleDataset(sample));
    ...
    List<GenericRow> expectedRows= testSampleDataset(sample).select("contig", "posStart", "alleles").javaRDD().map(x -> {
      return new GenericRow(new Object[]{Locus.apply((String)x.get(0), (int)x.get(1)), x.get(2)});
    }).collect();
    ...
    expectedRows= testSampleDataset(sample).select("contig", "posStart", "alleles").javaRDD().map(x -> {
      return new GenericRow(new Object[]{Locus.apply((String)x.get(0), (int)x.get(1)), x.get(2)});
    }).collect();
