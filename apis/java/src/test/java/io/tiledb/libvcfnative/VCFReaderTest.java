...
public class VCFReaderTest {
    ...
    private String[] getSamples() throws IOException {
        ...
        Object[] lines = FileUtils.readLines(new File(path.toAbsolutePath().toString()), "UTF8").toArray();
        String[] samples = new String[lines.length];

        for (int i=0; i<lines.length; ++i)
            samples[i] = (String)lines[i];
     ...
     * @throws IOException
     */
    private VCFReader getVFCReader(Optional<String[]> inputSamples, Optional<String> bedFile) throws IOException {
        String samples[] = inputSamples.orElse(getSamples());

        VCFReader reader = new VCFReader(constructUri("ingested_2samples"), samples,
                Optional.empty(), Optional.empty());

        if (bedFile.isPresent())
            reader.setBedFile(bedFile.get());
